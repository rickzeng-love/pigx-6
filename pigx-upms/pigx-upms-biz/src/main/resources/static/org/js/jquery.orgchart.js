  ...
  OrgChart.prototype = {
    ...
    createNode: function (data) {
      ...
      if (opts.nodeTemplate) {
       ...
       $nodeDiv.append('<div class="title" id="11' + data[opts.nodeId] + '">' + this.getStr(data[opts.nodeContent]) + '</div>')

        //更改的地方1
       // alert(data[opts.nodeId].substring(0,1));
        //alert(data[opts.nodeId].substring(1,1));
         if(data[opts.nodeId].substring(0,1)=='C'){
           $nodeDiv.append('<div class="content">'+
             '<div class="contbm">'+
             '<div class="fzr"><p>负责人：</p><span>'+this.getStr(data[opts.nodeName])+'</span></div>'+
             '<div class="renshu"><p>人数：</p><span>'+this.getStr(data[opts.nodeRS])+'</span></p></div>'+
             '</div>'+
             '<div class="contimg"><img src="http://39.99.202.116:9000'+data[opts.nodeImg]+'"/></div>'+
             '</div>');
         }else{
           $nodeDiv.append('<div class="content">'+
             '<div class="contbm">'+
             '<div class="fzr"><p>负责人：</p><span>'+this.getStr(data[opts.nodeName])+'</span></div>'+
             '<div class="renshu"><p>人数：</p><span>'+this.getStr(data[opts.nodeRS])+'</span></p></div>'+
             '<div class="bianz"><p>编制：</p><span>'+this.getStr(data[opts.nodeBZ])+'</span></p></div>'+
             '</div>'+
             '<div class="contimg"><img src="http://39.99.202.116:9000'+data[opts.nodeImg]+'"/></div>'+
             '</div>');
         }
      ...
      // $nodeDiv.append("<a href='javascript:void(0)' class='icon aiconadd' data-id='"+JSON.stringify(data)+"' onclick='addTu(this)'><i class='iconfont ante--add'></i></a>");
      // $nodeDiv.append("<a href='javascript:void(0)' class='icon aiconedit' data-id='"+JSON.stringify(data)+"' onclick='editTu(this)'><i class='iconfont ante--devise'></i></a>");
      // $nodeDiv.append("<a href='javascript:void(0)' class='icon aicondet' data-id='"+JSON.stringify(data)+"' onclick='deleteTu(this)'><i class='iconfont ante--delete'></i></a>");
